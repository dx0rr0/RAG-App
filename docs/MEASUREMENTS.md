...
- Añadido como opt-in y desactivado por defecto. Usa GPT-6 Luna `low`; limita la respuesta a 4.000 caracteres, la evidencia total a 8.000 caracteres y el timeout a 20 s. La primera versión fijó 64 tokens de salida; la evaluación posterior encontró outputs sin JSON y subió el límite a 512. Si se superan los límites o falla/falta el veredicto JSON booleano, la respuesta se rechaza (fail-closed).
...
### 2026-09-23 — Jev/Cohere ampliado y respuestas con citas

- Amplié a 24 preguntas (18 positivas, 6 negativas), 138 chunks, BM25 top 9 común a Jev y Cohere. Recall@3/MRR/nDCG@3 con evidencia alternativa revisada: BM25 `0.944/0.840/0.862`; BM25+Jev `1/1/1`; BM25+Cohere `1/1/1`. Cobertura BM25@9: `18/18`. Para q01/q07/q09 acepté respectivamente los chunks alternativos `{0,2}`, `{20,21}`, `{5,8}`: cada pregunta necesita un pasaje que responda, no dos fragmentos duplicados.
- Uso OpenRouter: Jev `$0.00236502` (24 consultas; `$0.0000985` por consulta); Cohere `$0.024` (24 búsquedas; `$0.001` cada una). Jev y Cohere empataron en la recuperación adjudicada; Jev cuesta ~10.1× menos. Jev scores máximos positivas `0.84–0.98`, negativas `0.02–0.10`; esta separación no calibra un umbral de producción.
- GPT-6 Luna `low`: 24 respuestas con top-3 Jev + un control BM25 para q10, más 25 verificaciones semánticas. El checker de esta medición usa un prompt experimental que trata una abstención explícita como apoyada; no es el prompt exacto del verificador de producción. Coste informado: generación `$0.0027837`; checker, incluidos los intentos truncados y sus verificaciones repetidas `$0.0037644`; total ampliado `$0.03291312`, debajo del límite `$0.04`. Revisión manual contra las transcripciones: 18/18 respuestas positivas respaldadas y 6/6 negativas abstuvieron claramente. q10 con BM25 top-3 no contenía el pasaje y no respondió; Jev lo puso en primer lugar.
- El checker experimental inicial agotó `max_tokens=64` en razonamiento en 13 casos. Repetí únicamente esos veredictos: 12 salieron con límite 256 y uno con 512. Resultado final: 20 veredictos verdaderos, 5 falsos y 0 malformados de 25; cuatro falsos son rechazos de respuestas positivas que la revisión humana considera apoyadas, uno es el rechazo esperado del control BM25. Esto no mide la tasa de error del verificador de producción.
- El gate de citas rechazó una abstención correcta sin cita («No puedo determinar»). Añadí un permiso estrecho para abstenciones claras sin cita; los hechos aún requieren cita, y referencias desconocidas se rechazan. Incrementé `FAITHFULNESS_MAX_TOKENS` a 512 y añadí tests. Suite local: `40/40` pasan.
- Total conocido OpenRouter antes/después: `$0.087125876` → `$0.120038996`; quedan aprox. `$6.879961` de los `$7` cargados. No se repitieron llamadas para volver a calcular métricas después del cambio de código.
- Artefactos: [resultados pareados](<C:/Users/Daniel Chorro/Documents/Codex/2026-09-22/ay/rag-eval-corpus-qwen-2026-09-23/expanded_reranker_comparison_results.json>), [RAG answers y grounding](<C:/Users/Daniel Chorro/Documents/Codex/2026-09-22/ay/rag-eval-corpus-qwen-2026-09-23/jev_rag_answer_evaluation.json>), [qrels alternativos](<C:/Users/Daniel Chorro/Documents/Codex/2026-09-22/ay/rag-eval-corpus-qwen-2026-09-23/adjudicated_evidence_variants.json>), [runner](<C:/Users/Daniel Chorro/Documents/Codex/2026-09-22/ay/rag-eval-corpus-qwen-2026-09-23/run_expanded_reranker_comparison.py>) y [runner de respuestas](<C:/Users/Daniel Chorro/Documents/Codex/2026-09-22/ay/rag-eval-corpus-qwen-2026-09-23/run_jev_rag_answer_eval.py>).

### 2026-09-23 — Integración optativa de Jev

- Cableado `--reranker jev` después de la fusión híbrida BM25+vectores. El reranker puntúa hasta 9 candidatos fusionados por defecto, en una única petición por consulta. El cross-encoder local existente y Jev son excluyentes; el camino predeterminado no llama a Jev.
- Se añadió caché local persistente indexada por SHA-256 de modelo, versión del prompt, consulta y candidatos. Solo guarda los scores; no conserva el texto enviado, la respuesta cruda ni la clave. Una respuesta Jev malformada también se marca para impedir reintentos facturables automáticos.
- Preflight conservador de coste con límite por consulta `$0.001`; si lo supera o el servicio falla, se mantiene el orden RRF y se informa en stderr. Jev no modifica el gate vectorial ni añade un umbral de suficiencia. No se realizaron llamadas a OpenRouter durante la implementación; el coste de integración fue `$0`.
- Validación local offline: suite `unittest` **50/50**, `cli_interface.py --help` exit 0 y `git diff --check` limpio. Los tests mockean el endpoint y comprueban caché, orden, límite de coste, errores y que el gate vectorial no use scores Jev.
- La evaluación de calidad sigue siendo la batería previamente medida: 24 consultas, BM25+Jev Recall@3/MRR/nDCG@3 `1/1/1`, coste reportado `$0.00236502`. La ruta integrada ahora usa candidatos híbridos y debe medirse de nuevo antes de afirmar esos resultados para producción.

### 2026-09-23 — Evaluación retrieval-only de la ruta híbrida integrada

- Comparación sobre los mismos 138 chunks y 24 consultas curadas (18 respondibles, 6 sin evidencia): BM25@20 + vectores@20 → RRF (`k=60`, candidatos fusionados=20) → Jev sobre top 9. Se empleó `baai/bge-m3` a través de OpenRouter; el proveedor respondió `parasail-bge-m3`. El runtime local no tiene PyTorch ni sentence-transformers, por lo que no se cargó localmente el mismo modelo. La división de 800/80 reproduce la configuración auditada del proyecto y generó 138 chunks.
- Recall@3/MRR/nDCG@3 con evidencia alternativa adjudicada: BM25 top 9 `0.944/0.840/0.862`; RRF top 9 `0.944/0.859/0.876`; RRF+Jev top 9 `1.000/1.000/1.000`. Los 18/18 positivos quedaron cubiertos por RRF top 9 antes de Jev; Jev mejoró el orden, sin añadir candidatos. Jev top 5 también obtuvo `1/1/1` en esta batería.
- Coste OpenRouter reportado: embeddings `162` entradas `$0.00023917`; Jev `24` consultas `$0.002385726`; total incremental **$0.002624896**, frente al tope de `$0.01`. No se llamó al generador ni al verificador de fidelidad. Jev latencia media 1.204 s, mediana 0.605 s, máximo 3.272 s.
- El gate vectorial actual `0.35` aceptó los 18 positivos y también los **6/6 negativos**, con RRF y con Jev. La máxima puntuación Jev fue `0.78–0.97` en positivos y `0.02–0.14` en negativos; los cortes observados de `0.3`, `0.5` y `0.7` separaron esta muestra completa. Esto sugiere evaluar Jev como señal adicional de abstención, pero seis negativas no calibran un umbral de producción.
- El primer intento local se detuvo después de guardar los seis lotes de embeddings y antes de llamar a Jev; corregí el postprocesamiento y reanudé desde checkpoint sin pagar de nuevo los embeddings. La evaluación completa terminó bajo el límite. Acumulado OpenRouter conocido actualizado: `$0.122663892` de los `$7` cargados (aprox. `$6.877336108` restantes; puede no incluir otro uso de la cuenta).
- Harness reanudable con checkpoint: [run_hybrid_jev_integration_eval.py](<C:/Users/Daniel Chorro/Documents/Codex/2026-09-22/ay/rag-eval-corpus-qwen-2026-09-23/run_hybrid_jev_integration_eval.py>). Resultados, scores y rankings (sin guardar pasajes): [hybrid_jev_integration_results.json](<C:/Users/Daniel Chorro/Documents/Codex/2026-09-22/ay/rag-eval-corpus-qwen-2026-09-23/hybrid_jev_integration_results.json>). Es una evaluación de retrieval, no una prueba de calidad de respuestas, citas, ACLs, carga ni disponibilidad del sistema.

### 2026-09-23 — RAG conversacional con historial y reformulación

- Situación base: `ask()` recibía solo la consulta actual y el bucle interactivo no guardaba turnos. La suite previa pasó **50/50** antes del cambio.
- Implementé un historial en memoria limitado por defecto a cuatro pares pregunta/respuesta, una acción `/clear` y `--history-turns 0` para desactivarlo. `ask()` conserva la consulta original para responder y citar, y usa una consulta independiente para buscar cuando la pregunta parece referirse a turnos anteriores. La respuesta solo puede fundamentarse en evidencia recuperada en el turno actual.
- La detección local no dispara reformulación para consultas autónomas. En una sonda pequeña preparada para esta función, detectó **5/5** seguimientos y dejó intactas **5/5** consultas autónomas. Es una prueba de humo de la heurística, no una estimación de su tasa de acierto en conversaciones reales.
