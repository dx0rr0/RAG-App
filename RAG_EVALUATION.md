...
Con el top 3 Jev probé `openai/gpt-6-luna`, `reasoning_effort=low`, sobre las 24 preguntas y añadí un control BM25-only para q10 (su gold estaba en BM25 rank 9, fuera del top 3). Usé el formateador de prompt y el validador de procedencia del repo; Jev se inyectó como ranking evaluado, pues aún no está integrado en la ruta de búsqueda de `ask()`. El checker semántico de esta medición usó el mismo modelo/esfuerzo, pero un prompt experimental que permite una abstención clara cuando el detalle no aparece; no es una evaluación idéntica al prompt del checker de producción. Revisé las respuestas contra las transcripciones.
...
- El checker experimental agotó inicialmente el límite de 64 tokens: 13 respuestas JSON quedaron vacías o parciales. Repetí solo esos veredictos con un límite mayor: 12 quedaron válidos en 256 tokens y el restante en 512. Resultado final: 20 `supported=true`, 5 `false`, 0 malformados de 25 casos. Cuatro rechazos son respuestas positivas que la revisión humana encontró respaldadas; el restante corresponde al control BM25 sin evidencia. Es una señal de que aún hay que calibrar el checker, no una tasa de error del checker de producción.
